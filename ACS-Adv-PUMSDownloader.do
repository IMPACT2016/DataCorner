/*******************************************************************************
** File name: C:/Users/palacios/OneDrive for Business/S/Training/gitHub/Posted/ACS_Download_161020.do
** Author(s): Vincent Palacios
** Posted Date: 10/20/16
** Description: 
** This Stata DO file will automatically download 2014 (or ealrier) 1-year national ACS microdata to your C:/P/ACS directory
** Version: 1.0
*******************************************************************************/
...
** FTP page for the ACS PUMS 2015 1-year files: 
...
** to download more than one year, separate years with a space (e.g. 2013 2014 2015). */
...
foreach calyear in `years' {
    ...
    if !_rc | "`overwrite_h'" == "1" {
        ...
        if "`state'"=="us" & `calyear' >= 2006 {
            local state usb
        }
        if `calyear' >= 2001 {
            local csvfile "ss`year'h`state'.csv"
        }
        else {
            local csvfile "c2ssh`state'.csv"
        }
        capture import delimited "`csvfile'", delimiter(comma) varnames(1) case(upper) clear               
        if _rc==199 {
            insheet using "`csvfile'", case comma clear
        }        
        else if _rc {
            error _rc
            exit
        ...
        **sort cy SERIALNO                                                      // #REMOVED: 8/26/16 to preserve original sort order
        ...
        notes: File downloaded on `c(current_date)' using ACS_Download_160829.do
...
foreach calyear in `years' {
    ...
    if !_rc | "`overwrite_p'" == "1" {
        ...
        if `calyear' >= 2001 {
            local csvfile "ss`year'p`state'.csv"
        }
        else {
            local csvfile "c2ssp`state'.csv"
        }
        capture import delimited "`csvfile'", delimiter(comma) varnames(1) case(upper) clear               
        if _rc==199 {
            insheet using "`csvfile'", case comma clear
